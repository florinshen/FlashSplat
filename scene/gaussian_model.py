...
class GaussianModel:
    ...
    def inpaint_setup(self, training_args, mask3d):
        ...
        with torch.no_grad():
            new_xyz, new_features_dc, new_scaling, new_features_rest, new_opacity, new_rotation = initialize_new_features(sub_features, num_new_points, mask_xyz_values)
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, limit_num=-1):
        ...
        # elif (limit_num > 0) and (self.get_num_pts >= limit_num):
        #     prune_mask = (self.get_opacity < min_opacity).squeeze()
        #     if max_screen_size:
        #         big_points_vs = self.max_radii2D > max_screen_size
        #         big_points_ws = self.get_scaling.max(dim=1).values > 0.1 * extent
        #         prune_mask = torch.logical_or(torch.logical_or(prune_mask, big_points_vs), big_points_ws)
        #     self.prune_points(prune_mask)
        #     self.densify_and_clone(grads[~prune_mask], max_grad, extent, limit_num=limit_num)
        #     self.densify_and_split(grads[~prune_mask], max_grad, extent, limit_num=limit_num)
