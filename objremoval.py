...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, 
    ...
    if all_counts is not None:
        ...
        obj_removal_path = os.path.join(model_path, name, "ours_{}".format(iteration), "removal{}".format(removal_name))
        ...
        for idx, view in enumerate(tqdm(views_used, desc="Rendering removal")):
            ...
            rendering = render_pkg["render"]
            torchvision.utils.save_image(rendering, os.path.join(obj_removal_path, '{0:05d}'.format(idx) + ".png"))
            # save gts
            gt = view.original_image[0:3, :, :]
            torchvision.utils.save_image(gt, os.path.join(gts_path, '{0:05d}'.format(idx) + ".png"))
        save_mp4(obj_removal_path, 3)
